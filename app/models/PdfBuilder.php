...
public function build(
		...
		$paths = $this->process_images($image_path,$pdf_path,$product_sub_type);
		if (!$paths)
		{
	 ...
	 * */
private function process_images($image_path,$pdf_path,$product_sub_type)	
	{
		try
		{
			$paths = [
				'full_size_image'	=> $this->process_pdf($pdf_path,'1.pdf',$product_sub_type),
...
private function process_pdf(
	...
	{	#$dest_filename = $dest_filename.'.pdf';
		#dd($pdf_path);
        if ( ! file_exists($pdf_path))
        {
            throw new \InvalidArgumentException();
        }
		if ($product_sub_type =='free'){
			$temp_path = public_path() //change to store as xxxx.pdf in public
            . DIRECTORY_SEPARATOR . 'temp' 
            . DIRECTORY_SEPARATOR . $dest_filename;
		#dd($temp_path);
		} else {
        ...
        return $pdf_path->move($temp_path,$pdf_path);
        //return $temp_path;
	}
}
